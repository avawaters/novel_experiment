...
simple.graph <- converted.data %>%
  ...
  summarize(average_incorrect = mean(correct), sd = sd(correct)) %>%
...
ggplot(simple.graph, aes(embed_level, average_incorrect)) +
  geom_pointrange(aes(ymin = average_incorrect-sd, ymax = average_incorrect+sd)) +
  labs(x = "Level of Embeddedness", y = "Average % of Questions Answered Incorrectly")
```

```{r Graph Linear Model attempt}
prediction.df <- ggpredict(mixed.lmer, terms = c("embed_level"))  

ggplot(prediction.df) + 
   ...
   geom_point(data = data.for.graph, aes(x = embed_level, y = incorrect, color = story)) + 
   labs(x = "Embeddedness Level", y = "Inorrect") + 
   theme_minimal()
```
